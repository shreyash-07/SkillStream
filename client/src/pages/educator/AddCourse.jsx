  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between
     md:p-8 md:pb-0 p-4 pt-8 pb-0'>
         ...
         onSubmit={handleSubmit}>
          ...
          <div className='flex items-center justify-between flex-wrap'>
            ...
            <div className='flex md:flex-row flex-col items-center gap-3'>
              ...
              <label htmlFor="thumbnailImage" className='fles items-center gap-3'>
                <img src={assets.file_upload_icon} alt="" className='p-3 bg-blue-500 rounded' />
                <input onChange={e => setImage(e.target.files[0])}
                 accept='image/*' type="file" id='thumbnailImage' hidden />
                    ...
                    {chapter.chapterContent.map((lecture, lectureIndex)=>(
                      <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                        <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration}
                          min - <a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a>\
        ...
        <button onClick={handleSubmit} type='submit' className='bg-black text-white w-max py-2.5 px-8 rounded my-4'>ADD</button>
    </div>
  )
}
